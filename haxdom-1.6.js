...
const token = "thr1.AAAAAGjEUjKr4IthXb3GCQ.RDUy-9OPZY0"; // https://www.haxball.com/headlesstoken
...
function narrateGoal(team, striker, assist) {
    try {
        const teamName = team === Team.RED ? 'rojo' :
            team === Team.BLUE ? 'azul' : 'desconocido';

        if (!striker) {
            room.sendAnnouncement(
                `[🎤] ¡Gol para el equipo ${teamName}!`,
                null, 0xA3FF00, null, HaxNotification.CHAT
            );
            return;
        }
        ...
        if (!pCompStriker) {
            room.sendAnnouncement(
                `[🎤] ¡Gol de ${striker.name || 'jugador desconocido'}!`,
                null, 0xA3FF00, null, HaxNotification.CHAT
            );
            return;
        }
        ...
        const playerGoals = Number(getGoalsPlayer(pCompStriker) || 0);
        const ownGoals = Number(getOwnGoalsPlayer(pCompStriker) || 0);
        const playerAssists = assist && pCompAssist ? Number(getAssistsPlayer(pCompAssist) || 0) : 0;

        const strikerName = striker.name || 'Jugador';
        const assistName = assist?.name || 'un compañero';
        let narration = '';
